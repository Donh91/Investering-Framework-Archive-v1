...
if(compass.data_status==='OK'){
  for(const h of ['NEXT_12H','NEXT_1_3D','NEXT_5_7D']) if(!compass.horizons?.[h]) throw Error(`missing ${h}`);
...
for(const token of ['MARKET COMPASS','NEXT 12 HOURS','NEXT 1–3 DAYS','NEXT 5–7 DAYS','Bitcoin → microcaps']) if(!renderer.includes(token)) throw Error(`renderer missing ${token}`);
...
console.log(JSON.stringify({status:'PASS',compass_id:compass.compass_id,data_status:compass.data_status}));
