...
No outcome may be scored before maturity. Missing outcome evidence remains missing and is not converted to zero or a miss. If no eligible target observation exists within the registered tolerance, the writer records `PENDING_TARGET_EVIDENCE` in its run result, writes no immutable outcome, and retries after evidence arrives.
...
No interpolation or synthetic price path is allowed. Target observations may use the nearest governed hourly close within ±2 hours; otherwise the target outcome remains pending. MFE and MAE stop exactly at the frozen horizon, even when an endpoint observation after the horizon is selected within the tolerance.
