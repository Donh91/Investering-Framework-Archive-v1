...
question: Does the official CN range or human adjustment beat DUMB_1.5 and DUMB_2.0 on forward verified rows?
start: CN_15
required_fields:
  - frozen_official_range
  - frozen_dumb_1_5_range
  - frozen_dumb_2_0_range
  - independently_verified_actual_high_low
  - winkler_scores
  - adjustment_alpha
rows_total: PENDING_REGISTRY_SYNC
valid_rows: PENDING_REGISTRY_SYNC
...
question: Does survival across ETHBTC, breadth, BTC.D, deployment and flow outperform first-cross logic?
required_fields:
  - ETHBTC_hold_days
  - breadth_state
  - BTC_D_state
...
required_fields:
  ...
  - timing_error
  - range_error
  - trade_result_if_applicable
  - framework_action_impact
rows_total: 0_SOURCE_BACKED_IMPORTED
valid_rows: 0
benchmark: simple_time_and_range_baselines
blocked_by: ORIGINAL_ISSUE_ROW_EXTRACTION
next_review: WEEKLY_IMPORT_STATUS
promotion_condition: enough original-source rows for category-specific calibration
kill_condition: none; reduce framework weight if calibration is poor
owner: RESEARCH_LAB
```
