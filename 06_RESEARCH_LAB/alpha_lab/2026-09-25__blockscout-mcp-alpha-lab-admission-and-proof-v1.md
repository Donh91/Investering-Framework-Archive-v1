...
\n## Operational repository integration\n\nThe repository now has a bounded exact-CA runtime adapter at `scripts/api_agent/meme_alpha_blockscout.py`. It implements the same source role defined above without creating a parallel scanner.\n\nTransport policy:\n- preferred unattended transport: unified Blockscout PRO API for chain `4663` using runtime-only `BLOCKSCOUT_API_KEY`;\n- bounded fallback: `robinhoodchain.blockscout.com/api/v2`;\n- launch discovery remains with canonical Pons/RPC owners;\n- ChatGPT interactive forensics may use the installed Blockscout connector directly.\n\nThe GitHub secret was live-tested on 2026-09-25. The unified PRO endpoint returned HTTP 200 for both the ASKR address endpoint and its creation transaction, including verified contract state and decoded `launchAndBuy` data. Credential-free proof is preserved in `research/api_agent/mcp/BLOCKSCOUT_ROBINHOOD_PRO_SMOKE_LATEST.json`.\n\nCredential handling rule: no key value in repository files, logs, receipts or persisted URLs. Source failure is `DEGRADED/UNAVAILABLE`, not evidence that an address/contract/transaction does not exist.\n