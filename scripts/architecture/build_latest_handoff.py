...
def main():
    ...
    handoff={'contract':'LATEST_HANDOFF_v2','generated_at_utc':now.isoformat().replace('+00:00','Z'),'pointers':pointers,'open_incidents':incidents,'pending_forecast_candidates':pending,'experiment_candidate_count':(experiment[2] or {}).get('candidate_count',0),'codex_ready_task_count':len((codex[2] or {}).get('tasks',[])),'authority':'READ_ONLY_ROUTING_SURFACE'}
    handoff['handoff_sha256']=hashlib.sha256(canon(handoff)).hexdigest();(r/'LATEST_HANDOFF.json').write_bytes(canon(handoff))
    lines=['# LATEST HANDOFF','',f"Generated: {handoff['generated_at_utc']}",f"Hash: `{handoff['handoff_sha256']}`",'']+[f"- **{k}**: `{(v or {}).get('path','UNAVAILABLE')}`" for k,v in handoff['pointers'].items()]+['',f"Open incidents: {len(incidents)}",f"Pending forecast candidates: {len(pending)}",f"Experiment candidates: {handoff['experiment_candidate_count']}",f"Codex-ready tasks: {handoff['codex_ready_task_count']}"]
    ...
    print(json.dumps({'status':'PASS','handoff_sha256':handoff['handoff_sha256'],'owners':len(owners),'experiment_candidates':handoff['experiment_candidate_count'],'codex_ready_tasks':handoff['codex_ready_task_count']},sort_keys=True))
