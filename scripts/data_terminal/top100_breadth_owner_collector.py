...
def fetch(url,timeout=20):
    req=urllib.request.Request(url,headers={"User-Agent":"Investering-Breadth-Owner/1.0","Accept":"application/json"})
    try:
...
def parse(payload):
    ...
    if not isinstance(rows,list) or len(rows)<100: raise E("INCOMPLETE_UNIVERSE",f"expected at least 100 rows, got {len(rows) if isinstance(rows,list) else 'non-list'}")
    ...
    for source_rank,row in enumerate(rows,1):
        ...
        seen.add(aid)
        if symbol in STABLE_SYMBOLS or "stablecoin" in str(row.get("categories","")).lower():
    ...
    if len(constituents)<80: raise E("INSUFFICIENT_FILTERED_UNIVERSE",str(len(constituents)))
    ...
    adv=sum(1 for r in constituents if r["change_24h_pct"]>0); dec=sum(1 for r in constituents if r["change_24h_pct"]<0); flat=len(constituents)-adv-dec
    aggregate={"constituent_count":len(constituents),"advancers":adv,"decliners":dec,"flat":flat,"advancer_pct":100*adv/len(constituents),"membership_hash":membership_hash}
...
def verify(root):
    ...
    owner=json.loads((root/"owner_snapshot.json").read_text()) if (root/"owner_snapshot.json").exists() else {}
    replay=owner.get("aggregate",{}).get("membership_hash")==sha(canonical([{"filtered_rank":r["filtered_rank"],"asset_id":r["asset_id"]} for r in owner.get("constituents",[])]))
    if not replay: failures.append({"path":"owner_snapshot.json","error":"MEMBERSHIP_REPLAY_MISMATCH"})
...
def run(payload,output,retrieval):
    ...
    owner={"contract":"C5E_TOP100_BREADTH_OWNER_v1","run_id":run_id,"retrieval_timestamp":retrieval,"freeze_timestamp":retrieval,"source":"COINGECKO_MARKET_CAP","ranking_metric":"market_cap_usd","method_version":"TOP100_FILTERED_STABLE_EXCLUSION_v1","constituents":constituents,"exclusions":exclusions,"aggregate":aggregate,"interpolation":False,"forward_fill":False,"authority":AUTHORITY}
    receipt={"run_id":run_id,"raw_sha256":sha(payload),"membership_hash":aggregate["membership_hash"],"constituent_count":len(constituents),"aggregate_replay":"PASS","status":"PASS","authority":AUTHORITY}
...
def main():
    ...
    try:
        if a.fixture: payload=a.fixture.read_bytes()
            ...
            q={"vs_currency":"usd","order":"market_cap_desc","per_page":100,"page":1,"sparkline":"false","price_change_percentage":"24h"}; payload=fetch(BASE+'?'+urllib.parse.urlencode(q))
        owner=run(payload,a.output_dir,retrieval); rb=verify(a.output_dir); print(json.dumps({"status":rb["status"],"run_id":owner["run_id"],"constituents":owner["aggregate"]["constituent_count"],"membership_hash":owner["aggregate"]["membership_hash"]},sort_keys=True)); return 0 if rb["status"]=='PASS' else 3
