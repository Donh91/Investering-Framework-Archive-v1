...
CANDIDATE_SET_SHA256 = "d470bf21cd79994ac59e97ddc4fd706944e0a613cd21dc362897d777d340f3d4"
...
def selection_schema(candidates: list[dict[str, str]]) -> dict[str, Any]:
    return {
        ...
        "properties": {"candidate_id": {"type": "string", "enum": [x["candidate_id"] for x in candidates]}},
    }
...
def validate_candidates(value: Any) -> list[dict[str, str]]:
    if value != FIXED_CANDIDATES:
        raise ValueError("candidate_set_differs_from_preregistered_v3")
    if base.sha(value) != CANDIDATE_SET_SHA256:
        raise ValueError("candidate_set_hash_mismatch")
    if len(value) != 10 or len({x["candidate_id"] for x in value}) != 10:
        raise ValueError("candidate_set_identity_invalid")
    for row in value:
        if parent.replayable_rule(_rule_only(row)) is None:
            raise ValueError(f"candidate_not_replayable:{row['candidate_id']}")
    return value
...
def validate_request(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw.get("selection_repeats") != 10 or raw.get("selection_max_output_tokens") != 200:
        raise ValueError("frozen_selection_runtime_mismatch")
...
def structured_selection(value: Any, candidates: list[dict[str, str]]) -> str | None:
    ...
    return str(candidate_id) if candidate_id in candidate_map(candidates) else None
...
def failure_class(record: dict[str, Any], selected_id: str | None) -> str | None:
    ...
    return None if selected_id is not None else "STRICT_SELECTION_SCHEMA_INVALID"


def fake_live_call(_api_key: str, payload: dict[str, Any], _model: str) -> dict[str, Any]:
    output = {"candidate_id": "C03"}
    return {
        "request_hash": base.sha(payload), "response_id": "dry-run", "output": output,
...
def evaluate_records(records: list[dict[str, Any]], *, request: dict[str, Any], design_summary: dict[str, Any], capability: dict[str, Any]) -> dict[str, Any]:
    ...
    structured_count = admitted_count = 0
    capability_snapshot = handoff.normalize_capabilities(capability)
    mission = {
        "experiment_id": request["experiment_id"], "issue": ISSUE, "request_sha256": base.sha(request),
        "phase": "DETERMINISTIC_CANDIDATE_SELECTION_ONLY", "candidate_set_sha256": CANDIDATE_SET_SHA256,
    }
    ...
    for repeat, record in enumerate(records, start=1):
        ...
        admitted = selected is not None and parent.replayable_rule(_rule_only(selected)) is not None
        admitted_count += int(admitted)
        contract = None
        if admitted and selected_id and selected:
            counts[selected_id] += 1
            artifact = {"candidate_id": selected_id, "rule": _rule_only(selected)}
            contract = handoff.build_contract(
                stage_id=f"E2_V3_SELECTION_RUN_{repeat}", mission=mission,
                input_artifact={"design_summary": design_summary, "candidate_set": candidates},
                capability_snapshot=capability, output_artifact=artifact,
                repair_scope=f"E2_V3_SELECTION_RUN_{repeat}", owner_candidate_id=request["experiment_id"],
                created_at=request["frozen_at_utc"],
            )
            handoff.assert_downstream_input(contract, artifact)
            capability_hashes.append(contract["capability_snapshot"]["snapshot_sha256"])
        failure = failure_class(record, selected_id)
        if failure:
            taxonomy[failure] += 1
        enriched.append({**record, "structured_selection_valid": selected_id is not None,
                         "candidate_admitted": admitted, "selected_candidate_id": selected_id,
                         "selected_candidate": selected, "failure_class": failure,
                         "handoff_id": contract.get("handoff_id") if contract else None})

    modal_id, modal_count = counts.most_common(1)[0] if counts else (None, 0)
    request_hashes = [str(row.get("request_hash") or "") for row in records]
    request_stable = len(set(request_hashes)) == 1 and bool(request_hashes[0])
    capability_stable = all(x == capability_snapshot["snapshot_sha256"] for x in capability_hashes)
    structured_rate, admission_rate, modal_share = structured_count / repeats, admitted_count / repeats, modal_count / repeats
    ...
    gates = {
        ...
        "identical_primary_payload": request_stable,
    ...
    return {
        ...
        "selection_entropy_bits": _entropy_bits(counts, admitted_count),
        "failed_call_count": sum(taxonomy.values()),
        "failure_taxonomy": dict(sorted(taxonomy.items())),
        "request_hash": request_hashes[0] if request_stable else None,
        "capability_snapshot_sha256": capability_snapshot["snapshot_sha256"],
        "candidate_set_sha256": base.sha(candidates),
        "gates": gates,
        "conclusion": "CANDIDATE_SELECTION_CONVERGENCE_SUPPORTED_FOR_ECONOMIC_SUCCESSOR" if supported else "CANDIDATE_SELECTION_CONVERGENCE_NOT_SUPPORTED",
        "economic_successor_eligible": supported,
    }


def run_trial(request: dict[str, Any], repo_root: Path, output_dir: Path, *, dry_run: bool = False,
              call_impl: Callable[[str, dict[str, Any], str], dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    envelope = {
        ...
        "design_summary": design_summary, "candidate_set_sha256": CANDIDATE_SET_SHA256,
        "candidate_set": candidates, "evaluation_outcomes_hidden": True,
        "economic_scoring_enabled": False, "direct_policy_arm_enabled": False,
    }
    if "_evaluation_label" in json.dumps(envelope):
        raise ValueError("evaluation_label_leak")
    payload = base.build_payload(request["model"], request["reasoning_effort"], "e2_candidate_selection_v3",
                                 schema, envelope, int(request["selection_max_output_tokens"]))
    capability = {
        "tools": ["openai.responses", "deterministic_candidate_validation", "agent_handoff_contract"],
        "schemas": {"selection_output": base.sha(schema), "candidate_set": CANDIDATE_SET_SHA256},
        "runtime": "AUTO_TRADING_E2_CANDIDATE_SELECTION_v3",
    }
    call_impl = call_impl or (fake_live_call if dry_run else base.live_call)
    ...
    result = {
        "contract": RESULT_CONTRACT, "status": "COMPLETE", "issue": ISSUE,
        "experiment_id": request["experiment_id"], "parent_experiment_id": request["parent_experiment_id"],
        "parent_result_sha256": request["parent_result_sha256"], "request_sha256": base.sha(request),
        "candidate_set_sha256": CANDIDATE_SET_SHA256, "source_hashes": source_hashes,
        "sample": {"design_rows": request["design_rows"],
                   "design_window": {"first_design_timestamp": design_summary["window"]["first_design_timestamp"],
                                     "last_design_timestamp": design_summary["window"]["last_design_timestamp"]},
                   "evaluation_outcomes_hidden_from_ai": True},
        "api": {"model": request["model"], "reasoning_effort": request["reasoning_effort"],
                "selection_calls": request["selection_repeats"], "failed_calls": evaluated["failed_call_count"],
        ...
        "selection": {key: evaluated[key] for key in (
            "structured_selection_success_rate", "candidate_admission_rate", "modal_candidate_share",
            "modal_candidate_id", "modal_candidate", "selection_counts", "selection_entropy_bits",
            "failure_taxonomy", "request_hash", "capability_snapshot_sha256", "candidate_set_sha256", "gates")},
        "conclusion": evaluated["conclusion"], "economic_successor_eligible": evaluated["economic_successor_eligible"],
        "economic_scoring_performed": False, "direct_policy_arm_performed": False, "capital_ready": False,
        "no_post_result_prompt_tuning": True, "no_retroactive_rescore": True, "authority": dict(AUTHORITY),
    }
...
def main() -> None:
    ...
    result = run_trial(json.loads(args.request.read_text(encoding="utf-8")), args.repo_root, args.output_dir, dry_run=args.dry_run)
    print(json.dumps({
        "experiment_id": result["experiment_id"], "status": result["status"], "conclusion": result["conclusion"],
