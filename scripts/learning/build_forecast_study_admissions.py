    ...
    for forecast_path in sorted(args.forecast_root.glob("*.json")) if args.forecast_root.exists() else []:
        try:
            ...
            try:
                ...
                write_new(
                    failure_path,
                    not_admitted(forecast, f"EVIDENCE_CLASS_ERROR:{exc}", activation, prereg_bytes, erratum_bytes, now),
                )
                counts["not_admitted_technical"] += 1
                continue

            if (
                evidence_class != OWNER_RATIFIED
                or forecast.get("direction") not in {"UP", "DOWN"}
                or forecast.get("settlement_contract_version") != EXACT_SETTLEMENT
            ):
            ...
            if len(candidates) != 1:
                write_new(
                    failure_path,
                    not_admitted(forecast, "SOURCE_CANDIDATE_UNIQUE_BINDING_REQUIRED", activation, prereg_bytes, erratum_bytes, now),
                )
                counts["not_admitted_technical"] += 1
                continue
            ...
            try:
                ...
                write_new(
                    failure_path,
                    not_admitted(forecast, f"SOURCE_TEMPORAL_PROVENANCE_FAILURE:{exc}", activation, prereg_bytes, erratum_bytes, now),
                )
                counts["not_admitted_technical"] += 1
                continue
            ...
            if source_path.exists() or b1_path.exists():
                ...
                try:
                    bars, source_doc = fetch_daily_history(
                        str(forecast["metric_path"]),
                        str(forecast["frozen_at_utc"]),
                        fixture=fixture,
                        min_bars=190,
                    )
                    source_doc.update(
                    ...
                    b1_core = b1_climatology(
                        bars,
                        str(forecast["frozen_at_utc"]),
                        int(forecast["horizon_days"]),
                        str(forecast["direction"]),
                        float(forecast["threshold_pct"]),
                    )
                    b1_doc = {
                    ...
                    write_new(
                        ...
                        not_admitted(
                            forecast,
                            f"B1_PRE_OUTCOME_TECHNICAL_FAILURE:{exc}",
                            activation,
                            prereg_bytes,
                            erratum_bytes,
                            now,
