...
def freeze_candidate(
    ...
    baseline_path: Path,
    output_root: Path,
    action_compass_receipt: Path | None = None,
...
def main() -> None:
    ...
    ap.add_argument("--action-compass-receipt", type=Path)
    args = ap.parse_args()
    candidate_record = load(args.candidate)
    packet = load(args.ratification)
    baseline = load(args.baseline_evidence)
    try:
        status, frozen, out = freeze_candidate(
            candidate_record,
            packet,
            baseline,
            args.baseline_evidence,
            args.output_root,
            args.action_compass_receipt,
        )
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    print(json.dumps({"status": status, "forecast_id": frozen["forecast_id"], "path": str(out)}, sort_keys=True))
