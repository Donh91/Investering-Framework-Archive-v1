...
def schema()->dict[str,Any]:
    return {"type":"object","additionalProperties":False,"required":["status","misses"],"properties":{"status":{"type":"string","enum":["READY","NO_SUPPORTED_MISSES"]},"misses":{"type":"array","maxItems":8,"items":{"type":"object","additionalProperties":False,"required":["phase","miss_type","decision_reference","outcome_reference","miss_description","proposed_metric_name","decision_relevance","missing_history_problem","desired_history_days","desired_cadence_minutes","data_shape","capability_hint","counterfactual_theory","confidence"],"properties":{"phase":{"type":"string","enum":PHASES},"miss_type":{"type":"string","enum":MISSES},"decision_reference":{"type":"string","minLength":1,"maxLength":300},"outcome_reference":{"type":"string","minLength":1,"maxLength":300},"miss_description":{"type":"string","minLength":3,"maxLength":500},"proposed_metric_name":{"type":"string","minLength":3,"maxLength":160},"decision_relevance":{"type":"string","minLength":3,"maxLength":500},"missing_history_problem":{"type":"string","minLength":3,"maxLength":500},"desired_history_days":{"type":"integer","minimum":1,"maximum":730},"desired_cadence_minutes":{"type":"integer","minimum":1,"maximum":10080},"data_shape":{"type":"string","enum":["TIME_SERIES","POINT_IN_TIME","EVENT_STREAM","DERIVED_SERIES","CROSS_SECTION"]},"capability_hint":{"type":"string","enum":HINTS},"counterfactual_theory":{"type":"string","minLength":3,"maxLength":500},"confidence":{"type":"string","enum":["LOW","MODERATE","HIGH"]}}}}}}

def call(key:str,ctx:dict[str,Any])->dict[str,Any]:
    inst=("You are a conservative decision-miss auditor for a shadow investment research system. Identify a miss only when supplied repository evidence contains a decision/forecast available before an outcome and a later outcome that supports the miss. Do not invent trades or claim the user should have acted. DATA_GAP_ONLY is preferred when evidence supports only missing information, not a demonstrated decision failure. For each supported miss, propose one observable evidence metric that might have reduced uncertainty. The miss episode is discovery-only and MUST NOT count as validation of that metric. No market rules, thresholds, weights, state or portfolio actions.")
    payload={"model":MODEL,"reasoning":{"effort":"low","context":"current_turn"},"store":False,"max_output_tokens":2200,"instructions":inst,"input":[{"role":"user","content":[{"type":"input_text","text":json.dumps(ctx,sort_keys=True)}]}],"text":{"format":{"type":"json_schema","name":"adaptive_decision_miss_v1","strict":True,"schema":schema()}}}
...
def main()->None:
    ...
    ctx={"contract":"ADAPTIVE_DECISION_MISS_AUDIT_INPUT_v1","documents":ds,"existing_evidence_gap_registry":gaps,"rules":["Discovery episode cannot validate its proposed metric.","Use only supplied repository evidence.","Prefer NO_SUPPORTED_MISSES over speculative hindsight."]}
    ...
    if cost>0.25:raise SystemExit(f"miss_audit_cost_exceeded:{cost}")
    gap_candidates=[]
