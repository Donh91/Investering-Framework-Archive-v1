...
def main():
    ...
    for path in a.daily_output_root.rglob('DAILY_DIRECTOR_OUTPUT.json'):
        ...
        output_hash=out.get('output_hash') or hashlib.sha256(canonical(out)).hexdigest()
        key=(when.isoformat(),output_hash)
        if key in seen:continue
        seen.add(key);candidates.append((when,path,out,receipt,output_hash))
    ...
    for day,row in sorted(by_day.items()):
        ...
        outputs.append({'local_day_key':day,'timezone':'Europe/Copenhagen','captured_at_utc':when.isoformat().replace('+00:00','Z'),'captured_at_local':when.astimezone(COPENHAGEN).isoformat(),'path':str(path),'output_sha256':hashlib.sha256(canonical(out)).hexdigest(),'receipt_sha256':hashlib.sha256(canonical(receipt)).hexdigest() if receipt else None,'output':out,'receipt':receipt})
    context={'contract':'WEEKLY_API_CALIBRATION_CONTEXT_v3','authority':'SHADOW_ONLY','iso_year':freeze['iso_year'],'iso_week':freeze['iso_week'],'evidence_timezone':'Europe/Copenhagen','window_start_utc':freeze['window_start_utc'],'window_end_utc':freeze['window_end_utc'],'freeze_sha256':freeze['freeze_sha256'],'weekly_capture_pack':weekly,'daily_director_rows':outputs,'daily_director_count':len(outputs),'selection_rule':'latest eligible row per Europe/Copenhagen local date within frozen local week, deduplicated by timestamp and output hash','handoff_targets':['RAW_WEEKLY_CALIBRATION','FORECAST_LEDGER','MASTER_MONDAY_PREP','SPECIALIST_REVIEW'],'rules':['Do not rewrite frozen forecasts.','Separate data quality from market evidence.','Preserve disagreement, missingness and censored outcomes.','Evaluate analysis and operational translation separately.','No framework-state, model-weight or portfolio authority.']}
