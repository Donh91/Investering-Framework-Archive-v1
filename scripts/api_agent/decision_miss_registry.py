...
def main()->None:
    ...
    reg.setdefault("contract","ADAPTIVE_DECISION_MISS_REGISTRY_v1");reg.setdefault("status","ACTIVE_SHADOW_RESEARCH_ONLY");reg.setdefault("items",{});reg.setdefault("authority",{"market_rule_change":False,"canonical_state":False,"portfolio_action":False,"automatic_sensor_promotion":False})
    ts=now();touched=[]
    for m in audit.get("misses",[]):
        if not isinstance(m,dict):continue
        mid="DM-"+key(m);prev=reg["items"].get(mid,{})
        reg["items"][mid]={"miss_id":mid,"phase":m.get("phase"),"miss_type":m.get("miss_type"),"decision_reference":m.get("decision_reference"),"outcome_reference":m.get("outcome_reference"),"miss_description":m.get("miss_description"),"proposed_metric_name":m.get("proposed_metric_name"),"counterfactual_theory":m.get("counterfactual_theory"),"confidence":m.get("confidence"),"first_seen_utc":prev.get("first_seen_utc") or ts,"last_seen_utc":ts,"observation_count":int(prev.get("observation_count",0) or 0)+1,"validation_semantics":"DISCOVERY_ONLY_DOES_NOT_VALIDATE_PROPOSED_METRIC","authority":{"evidence_only":True,"portfolio_action":False}}
        touched.append(mid)
