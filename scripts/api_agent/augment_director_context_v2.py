        ...
        try:
            with path.open(newline="", encoding="utf-8") as handle:
                for raw in csv.DictReader(handle):
                    ...
                    for field in NUMERIC_FIELDS: row[field] = to_float(raw.get(field))
                    row["btc_taker_buy_quote_share"] = to_float(raw.get("btc_taker_buy_quote_share")); row["eth_taker_buy_quote_share"] = to_float(raw.get("eth_taker_buy_quote_share"))
                    row["btc_high"] = to_float(raw.get("btc_high")); row["btc_low"] = to_float(raw.get("btc_low")); row["eth_high"] = to_float(raw.get("eth_high")); row["eth_low"] = to_float(raw.get("eth_low"))
                    rows.append(row)
        except Exception:
            continue
...
def select_anchor(rows: list[dict[str, Any]], target: datetime, max_lag_hours: float) -> dict[str, Any] | None:
    ...
    anchor = candidates[-1]; lag = (target - anchor["timestamp"]).total_seconds() / 3600.0
...
def build_horizon(rows: list[dict[str, Any]], cutoff: datetime, hours: int) -> dict[str, Any]:
    ...
    btc_taker=[v for v in (r.get("btc_taker_buy_quote_share") for r in window) if isinstance(v,float)]; eth_taker=[v for v in (r.get("eth_taker_buy_quote_share") for r in window) if isinstance(v,float)]; btc_flow=[v for v in (aggressive_flow(r,"btc") for r in window) if isinstance(v,float)]; eth_flow=[v for v in (aggressive_flow(r,"eth") for r in window) if isinstance(v,float)]
    return {"status":"READY","target_hours":hours,"anchor_timestamp_utc":anchor["timestamp"].isoformat().replace("+00:00","Z"),"latest_timestamp_utc":latest["timestamp"].isoformat().replace("+00:00","Z"),"actual_span_hours":round((latest["timestamp"]-anchor["timestamp"]).total_seconds()/3600.0,3),"sample_count":len(window),"btc_return_pct":pct_change(latest.get("btc_close"),anchor.get("btc_close")),"eth_return_pct":pct_change(latest.get("eth_close"),anchor.get("eth_close")),"ethbtc_return_pct":pct_change(latest.get("ethbtc_close"),anchor.get("ethbtc_close")),"btc_oi_change_pct":pct_change(latest.get("btc_open_interest"),anchor.get("btc_open_interest")),"eth_oi_change_pct":pct_change(latest.get("eth_open_interest"),anchor.get("eth_open_interest")),"btc_long_short_change_pct":pct_change(latest.get("btc_long_short_ratio"),anchor.get("btc_long_short_ratio")),"eth_long_short_change_pct":pct_change(latest.get("eth_long_short_ratio"),anchor.get("eth_long_short_ratio")),"btc_taker_buy_quote_share_mean":mean(btc_taker),"eth_taker_buy_quote_share_mean":mean(eth_taker),"btc_net_aggressive_quote_flow_sum":round(sum(btc_flow),6) if btc_flow else None,"eth_net_aggressive_quote_flow_sum":round(sum(eth_flow),6) if eth_flow else None,"btc_window_high":max(btc_highs) if btc_highs else None,"btc_window_low":min(btc_lows) if btc_lows else None,"eth_window_high":max(eth_highs) if eth_highs else None,"eth_window_low":min(eth_lows) if eth_lows else None}
...
def ethbtc_persistence(rows: list[dict[str, Any]], level: float = 0.03, lookback_hours: int = 168) -> dict[str, Any]:
    ...
    vals=[float(r["ethbtc_close"]) for r in valid]; latest=vals[-1]; side="ABOVE" if latest>level else "BELOW" if latest<level else "AT"; consecutive=0
...
def breadth_context(base: dict[str, Any]) -> dict[str, Any]:
    latest=base.get("latest_capture") if isinstance(base.get("latest_capture"),dict) else {}; previous=base.get("previous_capture") if isinstance(base.get("previous_capture"),dict) else {}; lm=latest.get("market_metrics",{}) if isinstance(latest.get("market_metrics"),dict) else {}; pm=previous.get("market_metrics",{}) if isinstance(previous.get("market_metrics"),dict) else {}; latest_ratio=breadth_ratio(lm); previous_ratio=breadth_ratio(pm)
    return {"latest_advance_ratio":latest_ratio,"previous_advance_ratio":previous_ratio,"advance_ratio_delta_pp":round((latest_ratio-previous_ratio)*100.0,4) if latest_ratio is not None and previous_ratio is not None else None,"source":"OWNER_CAPTURE_BREADTH_ONLY"}


def json_pointer_context(path: Path, source: str, fields: tuple[str, ...]) -> dict[str, Any]:
    if not path.exists(): return {"status":"UNAVAILABLE","reason":f"{source}_MISSING"}
    try: value=json.loads(path.read_text())
    except Exception: return {"status":"UNAVAILABLE","reason":f"{source}_INVALID"}
    out={key:value.get(key) for key in fields if key in value}; out["status"]="READY"; out["source"]=source
    return out
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("--context",type=Path,required=True); parser.add_argument("--hourly-root",type=Path,required=True); parser.add_argument("--etf-pointer",type=Path,default=Path("03_DAILY_CAPTURE_LOGS/etf/LATEST.json")); parser.add_argument("--pullback-pointer",type=Path,default=Path("03_DAILY_CAPTURE_LOGS/pullback_forensics/LATEST.json")); parser.add_argument("--rich-breadth-pointer",type=Path,default=Path("03_DAILY_CAPTURE_LOGS/breadth_rich/LATEST.json")); parser.add_argument("--stablecoin-pointer",type=Path,default=Path("03_DAILY_CAPTURE_LOGS/stablecoin_liquidity/LATEST.json")); parser.add_argument("--output",type=Path,required=True); args=parser.parse_args()
    ...
    context["api_intelligence_v2"]={"contract":"API_INTELLIGENCE_SEQUENCE_CONTEXT_v2_2","authority":"SHADOW_CONTEXT_ONLY","canonical_state":False,"cutoff_utc":cutoff.isoformat().replace("+00:00","Z"),"hourly_rows_available":len(rows),"horizons":{str(h):build_horizon(rows,cutoff,h) for h in HORIZONS_HOURS},"ethbtc_0_0300_persistence":ethbtc_persistence(rows),"breadth_delta":breadth_context(context),"rich_breadth":json_pointer_context(args.rich_breadth_pointer,"RICH_BREADTH_CHECKPOINT_v1",("retrieved_at_utc","aggregate","exclusion_count","authority")),"latest_settled_etf":settled_etf_context(args.etf_pointer),"pullback_forensics":pullback_forensics_context(args.pullback_pointer),"stablecoin_liquidity":json_pointer_context(args.stablecoin_pointer,"DEFILLAMA_STABLECOIN_LIQUIDITY_OWNER_v1_1",("retrieved_at_utc","global","historical_backfill","chains","source_receipts","payload_sha256","authority")),"rules":["All deltas are deterministic observations from retained owner/hourly data.","ETHBTC persistence uses direct retained ETHBTC closes and never a synthetic ETH/USD divided by BTC/USD ratio.","Net aggressive quote flow is taker-buy quote minus inferred taker-sell quote from the same exchange bar and is not labelled CVD.","Rich breadth is point-in-time cross-sectional evidence and missing historical rich fields are never reconstructed.","Stablecoin historical backfill is copied from the source historical series and is never interpolated or forward-filled.","Settled ETF context is copied only from verified retained ETF owner payload; missing sessions are not imputed.","Pullback Forensics remains SHADOW_RESEARCH_ONLY and cannot create market state or portfolio action.","Unavailable evidence remains unknown and is not imputed."]}
