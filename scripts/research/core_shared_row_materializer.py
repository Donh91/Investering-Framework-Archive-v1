        ...
        try:
            with p.open(newline="",encoding="utf-8-sig") as f:
                for r in csv.DictReader(f):
                    ...
                    try:e=float(r["ethbtc_close"]) if r.get("ethbtc_close") not in (None,"") else None
...
def latest_btcd(floor:datetime)->list[dict[str,Any]]:
    if not BTCD.exists():return []
    rows=[]
    with BTCD.open(newline="",encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            try:
                if r.get("source_status")!="PUBLIC_SOURCE_BACKED":continue
                if r.get("data_quality")!="PASS":continue
                if r.get("print_status")!="SETTLED_COMPLETE_DATE":continue
                if r.get("source_provider")!="CoinMarketCap":continue
                if "CMC_DIRECT_SOURCE_CONVENTION" not in str(r.get("source_convention","")):continue
                verified=parse_ts(r["source_verified_timestamp"])
                rows.append({"date":r["date_utc"],"value":float(r["btc_d_close"]),"provider":r.get("source_provider"),"convention":r.get("source_convention"),"verified_at_utc":iso(verified)})
            except Exception:continue
    latest=rows[-3:]
    if len(latest)<3:return []
    if any(parse_ts(x["verified_at_utc"])<floor for x in latest):return []
    return latest
...
def build(now_override:str|None=None)->dict[str,Any]:
    ...
    fr={x["family_id"]:x for x in freeze["families"]};core=["ETHBTC_PERSISTENCE","BREADTH_SURVIVAL","BTCD_PATH_RECLAIM"]
    if not all(fr[x].get("status")=="READY" and fr[x].get("candidate_decision_contract_status")=="READY" for x in core):return {"status":"NOT_ELIGIBLE","reason":"CORE_CONTRACT_NOT_READY"}
    hourly=load_hourly(wall)
    recent=hourly[-168:]
    valid=[x for x in recent if isinstance(x.get("ethbtc"),float)]
    if not valid:return {"status":"NOT_ELIGIBLE","reason":"DIRECT_ETHBTC_HOURLY_MISSING"}
    obs=valid[-1]["ts"]
    ...
    agg=breadth.get("aggregate") or {};adv,dec=agg.get("advancers"),agg.get("decliners")
    if not isinstance(adv,(int,float)) or not isinstance(dec,(int,float)):return {"status":"NOT_ELIGIBLE","reason":"BREADTH_ADVANCER_DECLINER_MISSING"}
    ...
    bt=latest_btcd(floor)
    if len(bt)<3:return {"status":"NOT_ELIGIBLE","reason":"BTCD_POST_FREEZE_OWNER_OR_THREE_SETTLED_PRINTS_MISSING"}
    ...
    latest=float(valid[-1]["ethbtc"]);side="ABOVE" if latest>0.03 else "BELOW" if latest<0.03 else "AT";consecutive=0
    ...
    regime,catalyst,ceid=catalyst_tags(obs);event_id="PSR_"+obs.strftime("%Y%m%dT%H%M%SZ")
    if event_id in read_ledger_ids():return {"status":"NOOP","reason":"EVENT_ALREADY_FROZEN","event_id":event_id}
    row={"event_id":event_id,"observation_timestamp_utc":iso(obs),"information_cutoff_utc":iso(obs),"source_version_commit":git_sha(),"regime_tag":regime,"catalyst_tag":catalyst,"catalyst_evidence_id":ceid,"ethbtc_raw_source":"BINANCE_DIRECT_ETHBTC_HOURLY","ethbtc_raw_value":latest,"ethbtc_window_inputs":canon({"lookback_rows":168,"sample_count":len(valid),"consecutive_same_side":consecutive,"method":"DIRECT_ETHBTC_HOURLY_CLOSES_NO_RATIO_SYNTHESIS"}),"ethbtc_derived_state":side,"ethbtc_missing":False,"breadth_membership_version":breadth.get("method_version") or breadth.get("contract") or "C5E_TOP100_BREADTH_OWNER_v1_2","breadth_membership_hash":bhash,"breadth_raw_inputs":canon({"retrieved_at_utc":bts,"advancers":adv,"decliners":dec,"flat":agg.get("flat"),"advance_ratio":agg.get("advance_ratio")}),"breadth_derived_state":br_state,"breadth_missing":False,"btcd_provider":"CoinMarketCap","btcd_denominator_version":"CMC_DIRECT_SOURCE_CONVENTION","btcd_raw_inputs":canon(bt),"btcd_derived_state":bd_state,"btcd_missing":False,"etf_missing":True,"leverage_missing":True,"stablecoin_missing":True,"cfgi_missing":True,"full_stack_missingness":canon({"status":"NOT_YET_ELIGIBLE","reason":"FULL_STACK_DECISION_CONTRACT_BLOCKED"}),"candidate_decisions":decisions,"preexisting_registered_outcomes":"ETHBTC_FORWARD_RELATIVE_RETURN_OUTCOME_v1"}
    return {"status":"ELIGIBLE_SHARED_ROW","row":row,"event_id":event_id,"candidate_decisions":decisions}

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--output",type=Path);ap.add_argument("--now-utc");ap.add_argument("--status-only",action="store_true");a=ap.parse_args();result=build(a.now_utc)
    if a.output and result.get("row") is not None:a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(result["row"],indent=2,sort_keys=True)+"\n")
