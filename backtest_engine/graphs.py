...
def validate_provenance_graph(nodes: dict[str, dict[str, Any]], edges: list[tuple[str, str]]) -> dict[str, Any]:
    order = topological_order(nodes, edges)
    errors: list[str] = []
    owner_nodes = {node_id for node_id, node in nodes.items() if node.get("authority_role") == "OWNER"}
    conclusion_nodes = {
    ...
    for conclusion in conclusion_nodes:
        if not ancestors(conclusion, edges).intersection(owner_nodes):
...
def latest_upstream_knowledge(nodes: dict[str, dict[str, Any]], edges: list[tuple[str, str]]) -> dict[str, str | None]:
    order = topological_order(nodes, edges)
    ...
    for node_id in order:
        ...
        for parent in parents[node_id]:
            if latest[parent] is not None:
                candidates.append(latest[parent])
...
def temporal_dependency_violations(nodes: dict[str, dict[str, Any]], edges: list[tuple[str, str]]) -> list[dict[str, str]]:
    latest = latest_upstream_knowledge(nodes, edges)
    violations: list[dict[str, str]] = []
    for node_id, node in nodes.items():
        decision_at = node.get("decision_at_utc")
        upstream_at = latest[node_id]
        if decision_at and upstream_at and _parse_utc(str(upstream_at)) > _parse_utc(str(decision_at)):
            violations.append({
                "node_id": node_id,
