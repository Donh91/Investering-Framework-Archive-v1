...
def connected_interval_clusters(
    intervals: Sequence[tuple[str, str]],
) -> tuple[int, ...]:
    """Assign clusters to intervals that overlap transitively."""
    parsed: list[tuple[int, datetime, datetime]] = []
    for index, (start_raw, end_raw) in enumerate(intervals):
    ...
    for original_index, start, end in ordered:
        ...
        assignments[original_index] = cluster
    return tuple(assignments)


def cluster_bootstrap_mean_ci(
    cluster_values: dict[str, Sequence[float]],
    replications: int,
    seed: int,
    alpha: float = 0.05,
) -> tuple[float, float, float]:
    """Bootstrap clusters, averaging within cluster before resampling."""
    ...
    for values in cluster_values.values():
        ...
        means.append(sum(numbers) / len(numbers))

    point = sum(means) / len(means)
    rng = random.Random(seed)
    samples = []
    for _ in range(replications):
        draw = [rng.choice(means) for _ in means]
    ...
    return point, quantile(alpha / 2.0), quantile(1.0 - alpha / 2.0)
