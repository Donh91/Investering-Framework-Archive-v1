...
| CHIEF_ACTION_COMPRESSION | ACTIVE_CONTRACT_RECEIPT_IMPLEMENTED | Uses Three-Horizon Action Compass v1.1 as the sole decision vocabulary, separates warning from action and persists one prospective immutable receipt per fresh ingest | 0 prospective Action Compass receipts at implementation | same immutable input packet / exact action-class agreement | NEEDS_ROWS | Governance / T9 | Modify if identical inputs yield materially inconsistent classes; never map warnings to actions or retired E0-E7 states |
