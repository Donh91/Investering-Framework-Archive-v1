...
class ForecastSettlementEligibilityTest(unittest.TestCase):
    ...
    def test_calibration_csv_stays_compatible_while_sidecar_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(eligibility_rows["exact"]["settlement_score_eligible"])
            self.assertFalse(eligibility_rows["exact"]["scientific_score_eligible"])
            self.assertEqual(eligibility_rows["exact"]["scientific_score_exclusion_reason"], "NONCANONICAL_FIXTURE_NO_SCIENTIFIC_AUTHORITY")
