...
## Bounded source-of-source chasing

When a load-bearing claim is supported only by a secondary source, trace the claim back toward its underlying primary record before treating the secondary source as sufficient.

Default bounded procedure:

1. Follow the cited or named upstream source once.
2. If that source is still secondary and the primary record is clearly identifiable, follow one additional hop.
3. Stop when the primary record is verified, the chain loops/repeats, the primary source cannot be identified, or the bounded retrieval budget is exhausted.
4. Use at most two citation-depth hops and three additional retrievals for one claim unless the task explicitly authorizes a larger investigation.
...
source_chain:
  ...
  hops: []
  terminal_source:
  terminal_type: PRIMARY | SECONDARY | UNKNOWN
  chase_status: PRIMARY_RECOVERED | BOUNDED_SECONDARY | LOOP_DETECTED | BLOCKED | NOT_APPLICABLE
```

A dead link, blocked page, paywall, API failure or absent archive is a coverage gap, not evidence that the cited claim is false. Never relabel a secondary paraphrase as primary merely because the original is unavailable.
