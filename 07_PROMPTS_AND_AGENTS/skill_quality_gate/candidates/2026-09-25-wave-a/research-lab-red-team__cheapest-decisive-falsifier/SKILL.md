...
## Falsifier ordering before research expansion

Before widening a surviving proposition into expensive research, identify its mandatory claim nodes and rank available falsifiers by **decisiveness per unit cost**.

Record:

```yaml
mandatory_claim_node:
candidate_falsifiers:
  - check:
    cost_class: FREE_LOCAL | CHEAP_API | BOUNDED_RESEARCH | EXPENSIVE_DEEP_DIVE
    decisive_if_failed: YES | NO
    evidence_required:
    unavailable_semantics: UNKNOWN
cheapest_decisive_falsifier:
stop_or_narrow_if_failed:
```

Rules:

- Prefer the cheapest check whose failure would make the current proposition non-promotable or materially narrower.
- A convenient check is not decisive merely because it is cheap.
- Source/provider unavailability, quota failure or missing evidence is `UNKNOWN`, never a failed falsifier.
- Do not spend on a broader deep dive while a cheaper decisive falsifier remains unresolved unless the broader work answers a separate material claim that cannot wait.
- If the falsifier fails, stop or narrow the proposition instead of inventing a rescue narrative.
- If it passes, treat that as survival of one node, not proof of the whole thesis.
...
For surviving proposals that require more research, include the selected `cheapest_decisive_falsifier`, its cost class and the exact stop/narrow condition before broader evidence work.
