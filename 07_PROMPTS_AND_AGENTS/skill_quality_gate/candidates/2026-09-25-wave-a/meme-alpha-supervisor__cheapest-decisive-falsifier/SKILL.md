...
### Pre-deep-dive falsifier gate

Before escalating a Meme Alpha case into broad web research, deep wallet graphing, Sol/Astra review or other materially more expensive work, identify the **cheapest decisive falsifier** for the current mandatory thesis node.

Prefer deterministic and already-authorized checks such as:

- exact CA / chain / creation-transaction verification;
- launch-origin or pair-token mismatch;
- first-party cross-bind failure where project ownership is a mandatory thesis node;
- sellability/liquidity impossibility where realizable alpha is the claim;
- point-in-time provenance failure where prospective credit is required.

The check must be capable of killing or materially narrowing the specific thesis. Cheap but non-decisive checks do not qualify.

If the check fails, stop or narrow the case before expensive expansion. If the check is unavailable, rate-limited or missing, preserve `UNKNOWN/DEGRADED` and continue only if the unresolved node justifies the cost. A passed falsifier means only that the case survives that node, not that it is a buy, legitimate project or proven edge.
