...
For every load-bearing corroboration claim, group sources by the **underlying evidence path**, not by domain count.
...
Treat sources as one independence group unless distinct evidence roots are demonstrated, including:

- syndicated copies of one wire story;
- multiple articles repeating one press release or official briefing;
- analyses built from the same dataset, cohort, benchmark or market feed;
- reports repeating the same anonymous source, social post or leaked document;
- model summaries or search snippets derived from the same underlying artifact.

Corroboration strength comes from distinct evidence roots. Ten URLs inheriting one root remain one root. Do not collapse genuinely independent primary evidence merely because it reaches the same conclusion.
