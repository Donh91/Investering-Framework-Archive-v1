# Astra BlockHorizon Research Mission Pointer v1

**Status:** NAVIGATION_ONLY_POINTER  
**Authority:** NONE_BY_ITSELF  
**Mission bundle:** `ASTRA_BLOCKHORIZON_HISTORICAL_FEATURE_SURVIVAL_V1`

## Why this exists

This file exists so an Astra-class or successor repository-aware model scanning `07_PROMPTS_AND_AGENTS/astra/` can discover the BlockHorizon research mission without a human re-pasting a chat prompt.

Do not execute from this pointer alone. Resolve current repository authority first.
...
Use the reconciled BlockHorizon historical BTC on-chain archive to determine which metrics genuinely add durable information beyond simpler baselines, which are redundant, regime-specific, hindsight-contaminated or untestable, and which surviving ideas deserve prospective testing in the existing framework.
...
point-in-time / revision-risk classification
redundancy and overlap map
incremental-value test design against simple baselines
pre-ETF vs ETF-era regime design
walk-forward + leave-one-cycle-out specification
...
- the archive is already large enough to start research preparation;
- 17 remaining Supply Last Active / Spent Volume family series are explicit gaps, not blockers;
- do not collect more BlockHorizon charts merely for completeness;
- the PlanB moving-average export contains post-retrieval model-grid timestamps that are not observed future actuals;
- private provider values remain in `Donh91/secrets` and must not be copied into the public control plane;
- do not create a new engine or parallel canonical owner.

## Discovery behavior

If you are Astra or a successor model and encounter this file during repository onboarding:
