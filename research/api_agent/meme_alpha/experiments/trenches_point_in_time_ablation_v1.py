...
EXPERIMENT = "TRENCHES_POINT_IN_TIME_G2_SOURCE_G3_ABLATION_v1"
...
G2_FIELDS = [
    ...
    "is_wash_trading",
    "age_seconds",
    "creator_funding_age_s",
    "tweet_age_s",
    "has_twitter",
    "has_telegram",
    "has_website",
    "has_fund_from_address",
]

# This is explicitly *not* clean reconstructed G3. It is a source-captured vendor
# annotation used only as a secondary arm to test whether a smart-wallet-like field
# adds anything above G2 in an independent forward capture.
...
def numericize(df: pd.DataFrame, cols: list[str]) -> list[str]:
    ...
    for c in cols:
        ...
        if int(x.notna().sum()) < max(100, len(df) // 200):
            continue
...
def timing_audit(features: pd.DataFrame) -> dict:
    ...
    if "age_seconds" in features.columns:
        a = pd.to_numeric(features["age_seconds"], errors="coerce")
    ...
    if {"tau__reach", "tau__reach_read"}.issubset(features.columns):
        ...
        if len(lag):
            out["reach_read_lag_ms"] = {
                ...
                "max": float(lag.max()),
                "negative_count": int((lag < 0).sum()),
            }
    out["forbidden_columns_present_but_excluded"] = sorted([
        c for c in features.columns
        if any(fragment in c.lower() for fragment in FORBIDDEN_NAME_FRAGMENTS)
    ])
    return out
...
def main() -> None:
    ...
    # One row per mint in features is documented, but preserve the earliest decision row if a future
    # release accidentally introduces duplicates.
    ...
    in_sample = set(split.get("in_sample", []))
    out_sample = set(split.get("out_of_sample", []))
    if not in_sample or not out_sample:
        raise RuntimeError("split.json missing in_sample/out_of_sample mint lists")

    train = panel[panel["mint"].isin(in_sample)].copy()
    test = panel[panel["mint"].isin(out_sample)].copy()

    g2 = numericize(panel, G2_FIELDS)
    sg3 = numericize(panel, SOURCE_G3_FIELDS)
    # Re-slice after numeric conversion so the model sees identical transformed columns.
    train = panel[panel["mint"].isin(in_sample)].copy()
    test = panel[panel["mint"].isin(out_sample)].copy()
    ...
    result = {
        ...
        "source_license": "PolyForm-Noncommercial-1.0.0 for the compilation/derived columns; public-ledger facts remain distinct",
        "outcome_semantics": {
            "path_completed": "chain-reading lifecycle completion label; NOT sellable return, NOT 5x/10x alpha"
        },
        "rows": {
        ...
        "interpretation_rules": [
            ...
            "Reach/X fields are excluded from admissible features because the release documents that they resolve after first-sighting decision time.",
    ...
    (out_dir / "trenches_point_in_time_ablation_v1.json").write_text(json.dumps(result, indent=2, sort_keys=True))

    def f(x):
        return "-" if x is None else f"{x:.4f}" if isinstance(x, float) else str(x)

    lines = [
        "# Trenches point-in-time ablation v1",
        "",
    ...
    for name, r in arms.items():
        if r.get("status") != "OK":
            ...
            lines.append(
                f"| {name} | {f(r.get('average_precision'))} | {f(r.get('roc_auc'))} | "
                f"{f(r.get('top_5pct_precision'))} | {f(r.get('top_5pct_lift'))} | {f(r.get('top_5pct_recall'))} |"
            )
    lines += [
        "",
        "## Guardrail",
        "",
        "A positive source-captured smart-degen delta cannot promote G3. It only decides whether the clean prospective wallet experiment remains worth pursuing.",
    ]
